...
\begin{description}
	...
	\item [\texttt{acknowledgements}] Acknowledgements.
        \item [\texttt{tabstract}] Abstract. This environment adds key-words to the end of the page, which can be written through a special argument. Check the section \ref{sec:env_example} for syntax.
\end{description}
...
\begin{tabstract}{Summary, Key-Words, \textit{Abstract}}
    Summary and Key-Words (in portuguese and english). The abstract describes the goals, the project's content and the conclusions. It has to be written in portuguese and english, with at most 250 words for each, and be next to 4-6 keywords. 
\end{tabstract}
\end{minted}
...
This chapter is dedicated to describing how the class and all it's functions were implemented inside the \TeX{} and \LaTeX{} context, with special care given to the \textit{packages} used. It has, therefore, a more technical nature, and it's reading is not required for any usage of the class.
...
In this section, a summary of all \textit{packages} used to implement the class can be found.
\begin{table}[ht]
	\centering
